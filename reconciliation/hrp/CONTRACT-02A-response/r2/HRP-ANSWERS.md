...
4. **Chứng minh Source (Xem chi tiết tại SOURCE-EVIDENCE.md):**
...
**Lưu ý về RLS và 404:**
- Source chứng minh `getLaborProfileDetail` dùng `findUnique`.
- Source chứng minh route map kết quả `null` thành 404.
- Migration chứng minh RLS được FORCE trên bảng.
- Vì vậy, việc RLS-hidden row dẫn đến `null` và trả về `404` là một **suy luận logic (inference) hợp lý** từ source và hành vi của PostgreSQL.
- Tuy nhiên, baseline hiện chưa có dedicated real-route integration test cho CRM/S2S path để chứng minh behavior này, nên đây KHÔNG ĐƯỢC TUYÊN BỐ là deployed CRM authorization evidence.

**Các kiểm chứng bổ sung (Negative Evidence):**
