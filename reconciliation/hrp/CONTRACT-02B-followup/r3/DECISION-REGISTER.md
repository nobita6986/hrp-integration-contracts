# Required Bilateral Owner Decisions (r3)
...
- **Exact HRP role/capability allowlist**: Strict mapping of effective user roles to endpoints. (HRP recommends strict initial parity with existing routes: `ADMIN`, `HR_MANAGER`, `HR_STAFF`. `HR_STAFF` remains under object-level RLS. `DIRECTOR`, `SALE`, `WORKER`, vendor roles, and `SERVICE`-only are strictly omitted. Any new capability like `CAN_READ_TALENT_CONTEXT` is a future implementation requirement, not in baseline).
...
*(Note: Known-but-unsupported projections requested by the consumer are not a denial condition. They are gracefully appended to `unavailableFields` in the response.)*
