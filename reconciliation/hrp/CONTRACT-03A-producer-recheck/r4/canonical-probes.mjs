...
const reviewedSha='272e883ef48c552054849905d7b75695b7ac4d8f';
...
function probe(id,expected,mutate=()=>{},rawHeader=JSON.stringify(header)) {
 ...
 mutate(input);
 let result;
 try {result=api.validateAssertionFromWire(rawHeader,{...input.opts,claims:input.claims});}
 catch(e){result={threw:e.message};}
 const pass=result.ok===expected;
 records.push({id,expected,actual:result.ok??null,pass,input:{rawHeader,...input},result});
...
probe('F01-A legacy flipped binding request rejected',false,x=>{
 x.claims.binding={...x.claims.request};
 x.claims.request={organizationId:binding.organizationId,crmSubject:binding.crmSubject,delegationRef:token('dg_')};
});
...
// Supplemental schema bounds use matching (but invalid) trusted context to
// distinguish format validation from equality-to-registration checks.
...
const summary={total:records.length,pass:records.filter(r=>r.pass).length,fail:records.filter(r=>!r.pass).length};
