# REC-002 - Request for HRP Proposals (T1-B)
...
## What CRM needs from HRP

### 1. Delegation Proof

Question: How does CRM receive a delegation proof from HRP?

Options that require HRP decision:
...
### 2. Binding Service - User - Organization - Request

Question: What is the exact binding model? Specifically:
...
Question: How does HRP handle revocation and retry?
...
### 4. Negative Acceptance Cases (Draft AC, not run)
...
- Specific algorithm (RS256 vs ES256 vs other)
- TTL value within the 60-second ceiling
...
- Role allowlist (the ADMIN/HR_MANAGER/HR_STAFF trio is HRP-illustrative only)
- Audit retention policy beyond metadata-only, no-PII principle

## Implication for role allowlist

CRM does NOT accept ADMIN/HR_MANAGER/HR_STAFF as the final role allowlist. HRP must:
- confirm whether this trio is complete for the slice,
- confirm whether new capabilities (e.g., CAN_READ_TALENT_CONTEXT) are required,
- provide the exact role / capability mapping table before implementation.

CRM explicitly does NOT open any endpoint based on this allowlist.