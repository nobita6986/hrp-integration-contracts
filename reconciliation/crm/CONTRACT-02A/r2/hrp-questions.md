...
Source: CRM CONTRACT-02A r2 (89dabd9 -> next revision)
...
- Client-domain timing (Q8): DEFERRED beyond current task scope.
- KPI/Phase10 ordering (Q9): DEFERRED beyond current task scope.
- Bilateral acceptance mechanism (Q5): already agreed per RECONCILIATION_PROTOCOL; HRP can use existing protocol directly.
...
Evidence reference:
- CRM side: ContextPanelResultSchema (queries.ts L312-352) defines the envelope shape including identitySummary, availability, currentRelationship, nextAction summary, contactability, suppressionSummary, and unavailableFields discriminator.
- CRM side: ReadOnlyIdentityPreviewRequestSchema (queries.ts L361-385) defines the read-only preview request shape.
- CRM side: QueryScopeSchema (queries.ts L100-115) defines organizationId required, actor required.
...
- HRP r3 GAP_REPORT has no entry directly mapping to ContextPanelResultSchema. This is a gap (see §5 of thin-slice-brief.md).
...
(d) Confirm which fields are always returned vs returned as unavailable.

Why CRM cannot proceed without it: The wire contract (ContextPanelResultSchema) exists on the CRM side; HRP has not confirmed whether it maps to an existing endpoint or a new one.
...
- CRM profile.ts defines PROFILE_PATCH_WHITELIST (L59-65) and update semantics (L1-130).
...
(d) Confirm whether the expectedVersion / snapshotVersion field is returned for optimistic concurrency.
...
Why this matters: If HRP enforces object-level permission server-side, CRM must pass the actor identity in the query scope. If CRM enforces it, HRP only sees the service identity. This affects the QueryScopeSchema.actor field.

### Q-A4. Response / error / version compatibility for read-only query

Decision-ID: NEW-Q-A4 (adjacent to REC-002 / REC-004b)

Question: What HTTP status codes and error shapes does HRP return for the read-only query? What happens when a field is unavailable vs when the query fails?

Evidence reference:
- CRM ContextPanelResultSchema (queries.ts L312) defines an unavailableFields discriminator (L338-346) listing fields that may be returned as unavailable without being errors.
...
(c) Confirm whether expectedVersion / snapshotVersion is monotonically increasing across reads (for cache invalidation).
...
Why this matters: If HRP returns the canonical-person suppression data, CRM can construct the union client-side. If HRP returns only the union result, CRM does not need to track the canonical source separately.
