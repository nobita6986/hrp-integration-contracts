 ...
 */
const TOKEN_GRAMMAR = /^[\p{L}][\p{L}\p{M}]*(?:['-][\p{L}][\p{L}\p{M}]*)*$/u;
/**
...
function firstLetterGrapheme(token) {
    ...
    if (firstCluster === undefined || firstCluster.length === 0)
        return null;
...
export function redactFullName(input) {
    ...
    if (containsControlOrFormat(input))
        return { success: false, reason: 'unsafe' };
