...
for (let i = 0x41; i <= 0x5A; i++)
    ALPHABET.add(String.fromCharCode(i)); // A-Z
for (let i = 0x61; i <= 0x7A; i++)
    ALPHABET.add(String.fromCharCode(i)); // a-z
for (let i = 0x30; i <= 0x39; i++)
    ALPHABET.add(String.fromCharCode(i)); // 0-9
ALPHABET.add('-');
ALPHABET.add('_');
const STD_ALPHABET = new Set([]);
...
export function encodeBase64Url(bytes) {
    ...
    if (typeof g.btoa === 'function') {
        ...
        std = g.Buffer.from(bytes).toString('base64');
    }
    else {
...
export function decodeBase64Url(str) {
    ...
    if (str.includes('='))
        throw new Error('contains padding');
    for (let i = 0; i < str.length; i++) {
        if (!ALPHABET.has(str[i]))
            throw new Error('non-alphabet character');
    }
...
export const OpaqueBindingLikeIdSchema = (max) => z
    .string()
    .min(1)
    .max(max)
...
function isValidGregorianDate(year, month, day, hour, minute, second) {
    if (month < 1 || month > 12)
        return false;
    ...
    if (second < 0 || second > 60)
        return false; // tolerate leap second.
    return true;
}
