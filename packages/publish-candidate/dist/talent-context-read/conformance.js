import { TalentContextReadFieldAllowlistSchema, } from './query-types.js';
...
export function checkResultConformance(requested, redactOutcome) {
    ...
    const reqAsTuple = reqParsed;
    for (const f of KNOWN_UNSUPPORTED) {
        if (reqAsTuple.indexOf(f) !== -1)
            unavailable.push(f);
    }
    const unavailableFields = UnavailableFieldsSchema.parse(unavailable);
 ...
 */
export function compareUnavailableFieldsInOrder(actual, expected) {
    if (!Array.isArray(actual) || !Array.isArray(expected))
        return false;
    if (actual.length !== expected.length)
        return false;
