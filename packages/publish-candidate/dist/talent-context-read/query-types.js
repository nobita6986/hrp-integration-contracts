function byteLengthUtf8(s) { if (typeof Buffer !== "undefined" && typeof Buffer.byteLength === "function")
    return Buffer.byteLength(s, "utf8"); return new TextEncoder().encode(s).length; }
import { z } from 'zod';
import { CorrelationIdSchema, OrganizationIdSchema, CanonicalIdSchema, DelegationRefSchema, IsoTimestampSchema, ModuleSchemaVersionSchema, } from './primitives.js';
// ============================================================================
...
export const IdentitySummarySchema = z
    ...
    fullNameRedacted: z
        .string()
        .min(1)
        .refine((s) => byteLengthUtf8(s) <= FULL_NAME_MAX_BYTES, 'fullNameRedacted exceeds 512 UTF-8 bytes'),
